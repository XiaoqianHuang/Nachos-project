...
SpaceId SysExec(char* exec_name) {
  char* fullName = kernel->fileSystem->getFullName(exec_name, kernel->currentThread->space->currentDirSector);
  if (fullName == NULL) {
    return -1;
  }
  ...
  if (kernel->fileSystem->canExecute(protectionFileBit) == FALSE) { //check file and dir protection bit
    cout << "Privilege Denied: file - " << protectionFileBit << "\n";
    return -1;
  }
  // end check privilege //
...
int SysJoin(int childid) {
  ...
  DEBUG(dbgSys, "child process " << childThread->getName() << " is joined to parent process " << kernel->currentThread->getName());
...
OpenFileId SysOpen(char *name, int mode) {
  ...
  if (kernel->currentThread->space->openFileTable->find(path) != kernel->currentThread->space->openFileTable->end()) { // existed in thread list
...
int SysClose(OpenFileId id) {
  ...
  string name = string(kernel->openFileTable->find(id)->second->getFullName());
  (*kernel->openNumForWR)[name] = (*kernel->openNumForWR)[name] - 1; // decrease the counter for accessing a file
...
int checkCMD(char *str) {
  ...
  if (cmd[0] == "ls") {
    ...
    char* filename = new char[cmd[1].length() + 1]; // arg1
    strcpy(filename, cmd[1].c_str());
    bool success = kernel->fileSystem->MakeDir(&filename, kernel->currentThread->space->currentDirSector); // fullpath will be output at filename
    if (success == FALSE) {
      return 0;
    }
