...
// Create a Nachos file of the same length
    ...
    if (!kernel->fileSystem->Create(to, fileLength, 1,7)) {   // Create Nachos file
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    userProgName->Append(argv[i + 1]); // add the user program to the pending executing list
	    i++;
	}
  ...
  else if (strcmp(argv[i], "-Q") == 0) { // parse the quantum
    ...
    kernel->Initialize(quantum, isRandom, useTLB);
    ...
    while (userProgName != NULL && userProgName->IsEmpty()  == FALSE) {
      Thread *t = new Thread("UserProg");
      t->Fork((VoidFunctionPtr)RunUserProg, (void*)userProgName->RemoveFront());
